...
import { HI, CAPS, type Lang } from './dub-data';
...
export function initDemo() {
  ...
  const loop = () => {
    ...
    const d = DV.hi.duration || HI.duration;
    const t = DV.hi.currentTime;
  ...
  // keep the English side in step with the (slightly longer) Hindi timeline
  DV.hi.addEventListener('ended', () => { pauseBoth(); });
  DV.en.addEventListener('ended', () => { DV.en.pause(); });
  ...
  playBtn.addEventListener('click', () => (DV.hi.paused ? playBoth() : pauseBoth()));
  q('[data-demo-restart]').addEventListener('click', restart);
  dialog.addEventListener('keydown', (e) => {
    if (e.key === 'ArrowLeft') setListen('en');
    if (e.key === 'ArrowRight') setListen('hi');
    if (e.key === ' ' && !(e.target instanceof HTMLButtonElement)) { e.preventDefault(); DV.hi.paused ? playBoth() : pauseBoth(); }
  });
}
